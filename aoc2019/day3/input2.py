...
XSPOTS = []
def update_grid(pos, char):
    gc = grid[pos[0]][pos[1]]
    if gc in ['|', '-'] and char in ['I', '=']:
        grid[pos[0]][pos[1]] = 'X'
        XSPOTS.append((pos[0], pos[1]))
    else:
        #if gc in ['o', NIL, '-', '|', '+']:
...
def render_grid(pos):
    sz = 80
    x0 = max((pos[0]-sz), 0)
    x1 = min((pos[0]+sz), len(grid[0]))
    y0 = max((pos[1]-sz), 0)
    y1 = min((pos[1]+sz), len(grid[1]))
    mini_grid = []
    # make it
    for line in grid[y0:y1]:
        mini_grid.append(line[x0:x1])
    # print it
    for i in range(len(mini_grid)):
        print(''.join(mini_grid[i]))
...
XDISTS = []
def check_x(pos, dist):
    gc = grid[pos[0]][pos[1]]
    if gc == 'X':
        XDISTS.append((copy.copy(pos), dist+1))


def go(dir, measuring=False, prev_dist=0):
    #print('so far:', dir, prev_dist)
    for i in range(abs(dir[0])):
        if dir[0] > 0:
            pos[0] += 1
        else:
            pos[0] -= 1
        if measuring:
            check_x(pos, prev_dist+i)
        else:
            update_grid(pos, V)
    for i in range(abs(dir[1])):
        if dir[1] > 0:
            pos[1] += 1
        else:
            pos[1] -= 1
...
qq = (start_pos[0], start_pos[1])
render_grid(qq)

dists = []
for spot in XSPOTS:
    dists.append((spot, dist(spot, start_pos)))
...
print("W1 SPOTS")
for dir in dirs1:
    go(dir, measuring=True, prev_dist=w1dist)
    w1dist += max([ abs(dir[0]), abs(dir[1]) ])
w1dists = copy.deepcopy(XDISTS)
...
w2dist = 0
for dir in dirs2:
    go(dir, measuring=True, prev_dist=w2dist)
    w2dist += max([ abs(dir[0]), abs(dir[1]) ])

sum_dists = []
for w2dist in XDISTS:  # which is w2 dists
    w2pos = w2dist[0]
    for w1dist in w1dists:
        w1pos = w1dist[0]
        if w2pos[0] == w1pos[0] and w2pos[1] == w1pos[1]:
            sum_dists.append(w1dist[1]+w2dist[1])
